...
{
	//[CompilerGenerated]
	public partial class ZoneSpawnSystemRe : GameSystemBase
	{
			...
			private bool SelectBuilding(ref SpawnLocation location, ref Random random, DynamicBuffer<ResourceAvailability> availabilities, ZoneData zoneData, float curvePos, float2 pollution, float landValue, float maxHeight, DynamicBuffer<ProcessEstimate> estimates, NativeList<IndustrialProcessData> processes, bool normal = true, bool storage = false, bool extractor = false, bool office = false)
			{
				...
				for (int i = 0; i < this.m_BuildingChunks.Length; i++)
				{
					...
					for (int j = 0; j < nativeArray3.Length; j++)
					{
                        ...
                        //if (num2 > location.m_Priority || propertyData.m_ResidentialProperties ==1)//´ýÑéÖ¤£»
						{
			...
			private int EvaluateDemandAndAvailability(Game.Zones.AreaType areaType, BuildingPropertyData propertyData, int size, bool storage = false)
			{
				...
				case Game.Zones.AreaType.Commercial:
				{
					...
					while (iterator2.Next())
					{
						...
						{
							num2 += this.m_CommercialDemands[EconomyUtils.GetResourceIndex(iterator2.resource)];
						}
					}
					return num2;
				}
		...
		protected override void OnCreate()
		{
			base.OnCreate();
            //log;
            Mod.log.Info("ZoneSpawnSystemRe Oncreated!");
			//
		...
		private bool CheckDemand(ref Random random, int demand)
		{
			return random.NextInt(5000) < demand * demand;
		}

		private bool CheckStorageDemand(ref Random random, int demand)
		{
			return demand > 0;
		}
		...
		{
			Random random = RandomSeed.Next().GetRandom(0);
			bool flag = this.debugFastSpawn || this.CheckDemand(ref random, this.m_ResidentialDemandSystem.buildingDemand.x + this.m_ResidentialDemandSystem.buildingDemand.y + this.m_ResidentialDemandSystem.buildingDemand.z);
			bool flag2 = this.debugFastSpawn || this.CheckDemand(ref random, this.m_CommercialDemandSystem.buildingDemand);
			bool flag3 = this.debugFastSpawn || this.CheckDemand(ref random, this.m_IndustrialDemandSystem.industrialBuildingDemand + this.m_IndustrialDemandSystem.officeBuildingDemand);
			bool flag4 = this.debugFastSpawn || this.CheckStorageDemand(ref random, this.m_IndustrialDemandSystem.storageBuildingDemand);
			...
			evaluateSpawnAreas.m_CommercialDemands = this.m_CommercialDemandSystem.GetBuildingDemands(out var deps);
